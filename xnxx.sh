...
config() {
    echo "xnxx在修改配置后会自动尝试重启"
    vi /etc/xnxx/config.json
    sleep 2
    restart
    check_status
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "检测到您未启动xnxx或xnxx自动重启失败，是否查看日志？[Y/n]" && echo
            read -e -rp "(默认: y):" yn
            [[ -z ${yn} ]] && yn="y"
            if [[ ${yn} == [Yy] ]]; then
               show_log
...
restart() {
    if [[ x"${release}" == x"alpine" ]]; then
        service xnxx restart
    else
        systemctl restart xnxx
    fi
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
...
status() {
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        systemctl status xnxx --no-pager -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
check_enabled() {
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        temp=$(systemctl is-enabled xnxx)
    ...
    node_config=$(cat <<EOF
{
            ...
            "CertConfig": {
                ...
                "CertFile": "/etc/xnxx/fullchain.cer",
                "KeyFile": "/etc/xnxx/cert.key",
                "Email": "xnxx@github.com",
                "Provider": "cloudflare",
                "DNSEnv": {
                    "EnvName": "env1"
                }
    ...
    node_config=$(cat <<EOF
{
            ...
            "NodeID": $NodeID,
            "NodeType": "$NodeType",
            "Hysteria2ConfigPath": "/etc/xnxx/hy2config.yaml",
            "Timeout": 30,
            "ListenIP": "",
            "SendIP": "0.0.0.0",
            "DeviceOnlineMinTraffic": 200,
...
generate_config_file() {
        ...
        cores_config+="
    {
        ...
        \"Log\": {
            \"Level\": \"error\",
            \"ErrorPath\": \"/etc/xnxx/error.log\"
        },
        \"OutboundConfigPath\": \"/etc/xnxx/custom_outbound.json\",
        \"RouteConfigPath\": \"/etc/xnxx/route.json\"
    },"
    fi
        ...
        cores_config+="
    {
        ...
        \"OriginalPath\": \"/etc/xnxx/sing_origin.json\"
    },"
    fi
...
EOF
    ...
    cat <<EOF > /etc/xnxx/custom_outbound.json
    [
        {
            "tag": "IPv4_out",
            "protocol": "freedom",
...
EOF
    ...
    cat <<EOF > /etc/xnxx/sing_origin.json
{
  "dns": {
    "servers": [
      {
...
EOF
    ...
    cat <<EOF > /etc/xnxx/hy2config.yaml
quic:
...
masquerade:
  type: 404
EOF
    echo -e "${green}xnxx 配置文件生成完成，正在重新启动 xnxx 服务${plain}"
    restart 0
    before_show_menu
}
...
————————————————
 ...
 #后续更新可加入上方字符串中
    ...
    case "${num}" in
        ...
        11) install_bbr ;;
        12) check_install && show_xnxx_version ;;
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        "version") check_install 0 && show_xnxx_version 0 ;;
